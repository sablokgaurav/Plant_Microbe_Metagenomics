...
sequences <- roary_pagoo[["align"]]
write.FASTA(sequences, file = "selected_genes.fasta", header = TRUE)
alignment <- readDNAStringSet(filepath = "./selected_genes.fasta")
alignment <- msa(alignment)
phylogeny <- system(paste("iqtree", "-s", "selected_genes.fasta" ,
                          "-p", "selected_genes.phy" ,"-m" , "MFP+MERGE", 
                          "-B" , "1000", sep = " "))
outlier <- odseq(alignment, distance_metric = "affine", B = 1000,
                 threshold = 0.025)
...
peppan_pca <- roary_pagoo$pan_pca()
