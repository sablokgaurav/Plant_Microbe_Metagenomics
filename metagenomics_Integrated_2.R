...
system2(roary)
core_roary_alignment <- readDNAMultipleAlignment(
               filepath = "./roary_processing/core_gene_alignment.aln",
               format = "DNA")
write.FASTA(core_roary_alignment, file = 
                      "./roary_processing/core_roary_alignment.fasta")
roary_core_phylogeny <- system2(command = "iqtree", 
...
core_fasta <- list.files(path = "./roary_processing/core_genes_roary.fasta")
dna_string <- readDNAStringSet(core_fasta)
dna_alignment_translation_based <- AlignTranslation(dna_string, verbose = FALSE)
dna_matrix <- DistanceMatrix(dna_alignment_translation_based, type = "dist")
core_roary_phylogeny <- TreeLine(myDistMatrix = dna_matrix, method = "complete", 
                                 cutoff = 0.05, showPlot = TRUE, reconstruct = TRUE)
roary_consensus <- ConsensusSequence(readDNAStringSet(core_fasta), ignoreNonBases = 
                                      TRUE, ambiguity = TRUE, ignoreNonBases = TRUE)
write.FASTA(roary_consensus, file = "./roary_processing/roary_consensus.fasta")
roary_mask <- MaskAlignment(readDNAStringSet(core_fasta),correction = TRUE,
                            showPlot = TRUE)
write.FASTA(roary_mask, file = "./roary_processing/core_genes_consensus_roary.fasta")
roary_staggered <- StaggerAlignment(AlignSeqs(readDNAStringSet(core_fasta)))
write.FASTA(roary_staggered, file = 
             "./roary_processing/roary_core_staggered_alignment.fasta")
consensus_fasta_tidy <- read.FASTA(file = "./roary/processing/core_genes_roary.fasta", 
                                   type = "DNA")
consensus_alignment <- tidy_msa( msa = consensus_fasta_tidy, start = 10, end = 70 )
consensus_alignment_plot <- treeMSA_plot(core_roary_phylogeny, consensus_alignment, 
                                         panel = "MSA", color = "Stanley_NT", 
                                         seq_colname = NULL)
gap_removal <- RemoveGaps(readDNAStringSet(core_fasta),
                          removeGaps = "all", processors = 3)
write.FASTA(gap_removal, file = "./roary_processing/core_genes_roary_gapless.fasta")
core_gapless <- list.files(path = "./roary_processing/core_genes_roary_gapless.fasta")
core_gapless_dna_string <- readDNAStringSet(core_gapless)
core_gapless_dna_alignment_translation_based <- 
 AlignTranslation(core_gapless_dna_string, verbose = FALSE)
core_gapless_dna_matrix <- DistanceMatrix(core_gapless_dna_alignment_translation_based, 
                                  type = "dist")
core_gapless_roary_phylogeny <- TreeLine(myDistMatrix = core_gapless_dna_matrix,
                                 method = "complete", cutoff = 0.05, showPlot = TRUE, 
                                 reconstruct = TRUE)
# #####################################################################
# #################### PEPPAN run #####################################
# #####################################################################
...
core_fasta <- list.files(path = "./peppan_processing/core_genes_peppan.fasta")
dna_string <- readDNAStringSet(core_fasta)
dna_alignment_translation_based <- AlignTranslation(dna_string, verbose = FALSE)
dna_matrix <- DistanceMatrix(dna_alignment_translation_based, type = "dist")
core_peppan_phylogeny <- TreeLine(myDistMatrix = dna_matrix, method = "complete", 
                                  cutoff = 0.05, showPlot = TRUE, reconstruct = TRUE)
peppan_consensus <- ConsensusSequence(readDNAStringSet(core_fasta), ignoreNonBases = 
                                       TRUE, ambiguity = TRUE, ignoreNonBases = TRUE)
write.FASTA(peppan_consensus, file = "./peppan_processing/peppan_consensus.fasta")
peppan_mask <- MaskAlignment(readDNAStringSet(core_fasta),correction = TRUE,
                             showPlot = TRUE)
write.FASTA(peppan_mask, file = "./roary_processing/core_genes_consensus_roary.fasta")
peppan_staggered <- StaggerAlignment(AlignSeqs(readDNAStringSet(core_fasta)))
write.FASTA(peppan_staggered, file = 
             "./peppan_processing/peppan_core_staggered_alignment.fasta")
consensus_fasta_tidy <- read.FASTA(file = "./peppan_processing/core_genes_peppan.fasta", 
                                   type = "DNA")
consensus_alignment <- tidy_msa( msa = consensus_fasta_tidy, start = 10, end = 70 )
consensus_alignment_plot <- treeMSA_plot(core_peppan_phylogeny, consensus_alignment, 
                                         panel = "MSA", color = "Stanley_NT", 
                                         seq_colname = NULL)
gap_removal <- RemoveGaps(readDNAStringSet(core_fasta),
                          removeGaps = "all", processors = 3)
write.FASTA(gap_removal, 
             file = "./peppan_processing/core_genes_peppan_gapless.fasta")
core_gapless <- list.files(path = 
                      "./roary_processing/core_genes_peppan_gapless.fasta")
core_dna_string <- readDNAStringSet(core_gapless)
core_gapless_dna_alignment_translation_based <- 
 AlignTranslation(core_dna_string, verbose = FALSE)
core_gapless_dna_matrix <- 
             DistanceMatrix(core_gapless_dna_alignment_translation_based, 
                                                                type = "dist")
core_peppan_gapless_phylogeny <- TreeLine(myDistMatrix = core_gapless_dna_matrix,
                             method = "complete", cutoff = 0.05, showPlot = TRUE, 
                                     reconstruct = TRUE)
#####################################################################
#################### PIRATE run #####################################
...
core_fasta <- list.files(path = "./pirate_processing/core_genes_pirate.fasta")
dna_string <- readDNAStringSet(core_fasta)
dna_alignment_translation_based <- AlignTranslation(dna_string, verbose = FALSE)
dna_matrix <- DistanceMatrix(dna_alignment_translation_based, type = "dist")
core_pirate_phylogeny <- TreeLine(myDistMatrix = dna_matrix, method = "complete", 
                                  cutoff = 0.05, showPlot = TRUE, reconstruct = TRUE)
pirate_consensus <- ConsensusSequence(readDNAStringSet(core_fasta), ignoreNonBases = 
                                       TRUE, ambiguity = TRUE, ignoreNonBases = TRUE)
write.FASTA(pirate_consensus, file = "./peppan_processing/peppan_consensus.fasta")
pirate_mask <- MaskAlignment(readDNAStringSet(core_fasta),correction = TRUE,
                             showPlot = TRUE)
write.FASTA(pirate_mask, file = "./pirate_processing/core_genes_consensus_pirate.fasta")
pirate_staggered <- StaggerAlignment(AlignSeqs(readDNAStringSet(core_fasta)))
write.FASTA(pirate_staggered, file = 
             "./peppan_processing/pirate_core_staggered_alignment.fasta")
consensus_fasta_tidy <- read.FASTA(file = "./pirate_processing/core_genes_pirate.fasta", 
                                   type = "DNA")
consensus_alignment <- tidy_msa( msa = consensus_fasta_tidy, start = 10, end = 70 )
consensus_alignment_plot <- treeMSA_plot(core_pirate_phylogeny, consensus_alignment, 
                                         panel = "MSA", color = "Stanley_NT", 
                                         seq_colname = NULL)
gap_removal <- RemoveGaps(readDNAStringSet(core_fasta),
                          removeGaps = "all", processors = 3)
write.FASTA(gap_removal, file = "./pirate_processing/core_genes_pirate_gapless.fasta")
core_gapless <- list.files(path = "./pirate_processing/core_genes_pirate_gapless.fasta")
core_dna_string <- readDNAStringSet(core_gapless)
core_gapless_dna_alignment_translation_based <- 
 AlignTranslation(core_dna_string, verbose = FALSE)
core_gapless_dna_matrix <- 
     DistanceMatrix(core_gapless_dna_alignment_translation_based, type = "dist")
core_pirate_gapless_phylogeny <- TreeLine(myDistMatrix = core_gapless_dna_matrix,
                             method = "complete", cutoff = 0.05, showPlot = TRUE, 
                                        reconstruct = TRUE)
#####################################################################
